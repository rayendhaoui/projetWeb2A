...
require_once '../Entities/reponse.php'; // Importation de la classe ReponseReclamation
...
class reponseC
{
    public function ajouterReponse($reponse) {
        try {
            ...
            $sql = "INSERT INTO reponse(id_rep, reclaimID, message, date ,email) VALUES (:id_rep, :reclaimID, :message, :date, :email)";
    ...
    public function getReponseById($id_rep) {
        try {
            $db = config::getConnexion();
            $stmt = $db->prepare("SELECT * FROM reponse WHERE id_rep = :id_rep");
